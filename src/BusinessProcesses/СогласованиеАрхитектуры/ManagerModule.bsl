...
Функция АктивныйПроцесс(ЗадачаМетеор, ПроцессИсключения = Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СогласованиеАрхитектуры.Ссылка КАК Ссылка
	|ИЗ
	|	БизнесПроцесс.СогласованиеАрхитектуры КАК СогласованиеАрхитектуры
	|ГДЕ
	|	СогласованиеАрхитектуры.Стартован
	|	И НЕ СогласованиеАрхитектуры.Завершен
	|	И НЕ СогласованиеАрхитектуры.ПометкаУдаления
	|	И СогласованиеАрхитектуры.ЗадачаМетеор = &ЗадачаМетеор";
	Запрос.УстановитьПараметр("ЗадачаМетеор", ЗадачаМетеор);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		АктивныйПроцесс = Выборка.Ссылка;
	Иначе
		АктивныйПроцесс = БизнесПроцессы.СогласованиеАрхитектуры.ПустаяСсылка();
	КонецЕсли;
