...
Процедура ОтправитьОповещениеСвязаннымОтветственнымПоОбъектам()
	...
	Пока Выборка.СледующийПоЗначениюПоля("ОтветственныйПоПодсистеме") Цикл
		...
		ЧастиСообщения.Добавить(ТемаПисьма + ":");
		Пока Выборка.СледующийПоЗначениюПоля("ВерсияХранилища") Цикл
			...
			ЧастиПисьма.Добавить(
				СтрШаблон("<a href=""%3"">Коммит %1 (%2)</a>",
				Выборка.ВерсияХранилищаПредставление,
				Выборка.КонфигурацияПредставление,
				РаботаСGIT.URLGit(Выборка.ВерсияХранилища)));
			ЧастиПисьма.Добавить(
				СтрШаблон("Изменены: %1",
				СтрСоединить(ОбъектыВерсии, ", ")));
			ЧастиПисьма.Добавить(
				СтрШаблон("Проверил код: %1",
				Выборка.Проверил));
		КонецЦикла;
		
		ТекстСообщения = СтрСоединить(ЧастиСообщения, Символы.ПС);
		ЧастиПисьма.Добавить("</body></html>");
		ТекстСообщенияПисьмо = СтрСоединить(ЧастиПисьма, "<br>");
		// Отправка по и-мейл
...
Функция ОписаниеДанныхОповещенияРезультатаПроверки()
	Результат = Новый Структура;
	Результат.Вставить("МассивОписанияЭлектронныхПисем");	
	Результат.Вставить("МассивОписанияОповещнийТелеграмм");
