...
Функция СообщенияКОтправке()
	...
	Пока Выборка.СледующийПоЗначениюПоля("ОтветственныйПоПодсистеме") Цикл
		...
		ЧастиПисьма.Добавить(ТемаПисьма + " по коммитам:");
		...
		Пока Выборка.СледующийПоЗначениюПоля("ВерсияХранилища") Цикл
			...
			ЧастиПисьма.Добавить(СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонПисьма(), Изменение));
			
		КонецЦикла;
		
		ДанныеРассылки.ТелеграмСообщение = СтрСоединить(ЧастиТелеграмСообщения, Символы.ПС);
		ЧастиПисьма.Добавить("</body></html>");
