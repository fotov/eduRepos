...
Процедура ПеревестиНаСледующийЭтапПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Если Не ИнтеграцияСМетеор.ПроверитьТекущийЭтапЗадачиВМетеоре(ЗадачаМетеор, Справочники.ЭтапыЗадачиМетеор.ОтключениеРасширения) Тогда
		Возврат;
	КонецЕсли;
	
	Этапы = РегистрыСведений.СледующиеЭтапыЗадачиПоТочкеМаршрута.ПолучитьЭтапыПоТочкеМаршрута(
		Бизнеспроцессы.ОтключениеРасширения.ТочкиМаршрута.ПеревестиНаСледующийЭтап,
		Согласовано);
...
Процедура ОтключитьРасширениеПриВыполнении(ТочкаМаршрутаБизнесПроцесса, Задача, Отказ)
	...
	Попытка
		ПараметрыУдаления = Новый Структура("Имя");
		Пока Выборка.Следующий() Цикл
			ПараметрыУдаления.Имя = Выборка.Имя;
			РаботаСДополнениями.УдалитьДополнение(Выборка.База, Перечисления.ТипыДополнений.Расширение, ПараметрыУдаления);
			СообщенияОВыполнении.Добавить(СтрШаблон("Отключено %1 в базе %2", Выборка.ВерсияДополненияПредставление, Выборка.БазаПредставление));
		КонецЦикла;
		Согласовано = Истина;
		Записать();
	Исключение
	...
	КонецЦикла;
	СостояниеДополненийВБазах = РаботаСДополнениями.ПолучитьСостояниеДополнений(Перечисления.ТипыДополнений.Расширение, Базы);
	ОтборБазаРасширение = Новый Структура("База, Имя", Неопределено, Неопределено);
	Выборка.Сбросить();
	Пока Выборка.Следующий() Цикл
		...
		Если УстановленныеРасширения.Количество() Тогда
			СообщенияОВыполнении.Добавить(СтрШаблон(
				"Ошибка: После отключения расширения %1 в базу %2 - обнаружено подключенное активное расширение. (при отключении не было исключений)",
				Выборка.ВерсияДополненияПредставление,
				Выборка.БазаПредставление));
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;	
