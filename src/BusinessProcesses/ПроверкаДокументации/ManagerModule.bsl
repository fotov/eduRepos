...
Функция АктивныйПроцесс(ЗадачаМетеор, ПроцессИсключения = Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПроверкаДокументации.Ссылка КАК Ссылка
	|ИЗ
	|	БизнесПроцесс.ПроверкаДокументации КАК ПроверкаДокументации
	|ГДЕ
	|	ПроверкаДокументации.Стартован
	|	И НЕ ПроверкаДокументации.Завершен
	|	И НЕ ПроверкаДокументации.ПометкаУдаления
	|	И ПроверкаДокументации.ЗадачаМетеор = &ЗадачаМетеор";
	Запрос.УстановитьПараметр("ЗадачаМетеор", ЗадачаМетеор);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		АктивныйПроцесс = Выборка.Ссылка;
	Иначе
		АктивныйПроцесс = БизнесПроцессы.ПроверкаДокументации.ПустаяСсылка();
	КонецЕсли;
