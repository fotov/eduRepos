...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ДополнительныеСвойства.Свойство("ОтключитьЗаполнение_email")
			И (ЭтоНовый Или Не ПустаяСтрока(АдресЭП)) Тогда
		email = АдресЭП;
	КонецЕсли;
	
	// +++АК DORS 2023.06.14 Реквизит "ПользовательОС" используется для выгрузки в SQL коннект и АПК
	СвойстваПользовательИБ = Пользователи.СвойстваПользователяИБ(ИдентификаторПользователяИБ);
	Если СвойстваПользовательИБ <> Неопределено И СвойстваПользовательИБ.АутентификацияОС И ЗначениеЗаполнено(СвойстваПользовательИБ.ПользовательОС) Тогда
		ПользовательОС = СвойстваПользовательИБ.ПользовательОС;
	Иначе
		ПользовательОС = "";
	КонецЕсли;
	// ---
