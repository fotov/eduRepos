...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ПерсональныйТокенGitLab = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(Объект.Ссылка, "ПерсональныйТокенGitLab");
		ПерсональныйТокенGitLab = ?(ЗначениеЗаполнено(ПерсональныйТокенGitLab), ЭтотОбъект.УникальныйИдентификатор, "");
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПерсональныйТокенGitLabИзменен И Не Отказ Тогда
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, ПерсональныйТокенGitLab, "ПерсональныйТокенGitLab");
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// +++
	Если Не БлокировкаРаботыСВнешнимиРесурсамиУКПовтИсп.ЭтоКопияБазы() Тогда
		ОбновлениеAUTHORSСервер.ВыполнитьОбновлениеФайловAUTHORSПоПараметрам(Объект.Ссылка);
	КонецЕсли;
	// ---
...
&НаКлиенте
Процедура ПерсональныйТокенGitLabПриИзменении(Элемент)
	ПерсональныйТокенGitLabИзменен = Истина;
КонецПроцедуры
