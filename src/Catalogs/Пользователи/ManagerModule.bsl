...
#Область АК_Пользователи

// +++
Функция ПользователиРоли(ОсновнаяРоль) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РегистрАдресации.Исполнитель
	|ИЗ
	|	РегистрСведений.РегистрАдресации КАК РегистрАдресации
	|ГДЕ
	|	РегистрАдресации.РольИсполнителя = &ОсновнаяРоль";
	Запрос.УстановитьПараметр("ОсновнаяРоль", ОсновнаяРоль);
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку(0);
	
КонецФункции

// +++
Функция ТелеграмIDПользователей(Пользователи) Экспорт
	
	ТелеграмIDПользователей = Новый Массив;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.ТелеграмID) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Лев(Выборка.ТелеграмID, 1) = "@" Тогда
			ТелеграмIDПользователей.Добавить(СтрШаблон("%1 (%2)", Выборка.ТелеграмID, Выборка.Пользователь));
		Иначе
			ТелеграмIDПользователей.Добавить(СтрШаблон("[%1](tg://user?id=%2)", Выборка.Пользователь, Выборка.ТелеграмID));
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ТелеграмIDПользователей;
