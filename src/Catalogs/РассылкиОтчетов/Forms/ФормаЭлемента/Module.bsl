...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	// +++ Турков, 2023.04.21
	Если СтрНайти(ТекстПисьмаФорматированныйДокумент.ПолучитьТекст(), "[СформированныеОтчеты]") Тогда
		Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонСформированныхОтчетов.Пометка = Истина;
		УстановитьДоступностьПараметровШаблонаТекстаПисьма(ЭтотОбъект, Истина);
	КонецЕсли;
	// ---
КонецПроцедуры
...
Процедура ДобавитьШаблонСформированныхОтчетов(Команда)
	// +++ Турков, 2023.04.21
	Если Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонСформированныхОтчетов.Пометка Тогда
		ТекстПисьмаФорматированныйДокумент.Удалить();
		Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонСформированныхОтчетов.Пометка = Ложь;
		УстановитьДоступностьПараметровШаблонаТекстаПисьма(ЭтотОбъект, Ложь);
	Иначе
		ТекстВопроса = "Шаблон письма будет очищен. Сформированные отчеты будут помещены в тело письма. Продолжить?";
		Ответ = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена,,, "Добавление отчетов в тело письма");
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьПараметровШаблонаТекстаПисьма(Форма, ФормироватьПредставлениеОтчетовВТексте)
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьИлиИзменитьШаблонДатыРассылки.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонАвтора.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонПолучателя.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонРассылки.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонСистемы.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонСпособаДоставки.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументДобавитьШаблонПоУмолчанию.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ТекстПисьмаФорматированныйДокументПредпросмотрШаблона.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	Форма.Элементы.ГруппаПараметрыАрхивацииВZIP.Доступность = Не ФормироватьПредставлениеОтчетовВТексте;
	
	Форма.Элементы.ТекстПисьмаФорматированныйДокумент.ТолькоПросмотр = ФормироватьПредставлениеОтчетовВТексте; // Изменение доступности ломает добавление шаблона.
КонецПроцедуры
