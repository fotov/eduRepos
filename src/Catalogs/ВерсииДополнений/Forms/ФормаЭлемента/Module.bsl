...
Асинх Процедура ОбновитьИзФайлаПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если Не Отказ И ТипДополнения = ПредопределенноеЗначение("Перечисление.ТипыДополнений.Расширение")
			И Не ИмяСоответствуетШаблону(ОписаниеЗагруженнойВерсии.Имя, КомментарийОшибки) Тогда
		МассивОшибок.Добавить(
			СтрШаблон(
				"Для загружаемого дополнения проверка правила именования завершилась с ошибкой (%1).
				|Установите корректное имя расширения. %2", 
				КомментарийОшибки,
		...
		ТекстВопроса = СтрШаблон(
			"Для загружаемого дополнения проверка правила именования завершилась с ошибкой (%2). Выберите действие. %1",
			ОписаниеЗагруженнойВерсии.Имя, КомментарийОшибки);
...
&НаСервереБезКонтекста
Функция ИмяСоответствуетШаблону(Имя, КомментарийОшибки)
