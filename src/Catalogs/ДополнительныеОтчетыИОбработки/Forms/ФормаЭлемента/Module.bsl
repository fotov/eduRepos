...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +++ tura, 2023.01.18
	// Добавляем команду и кнопку на форму
	ИмяКоманды = "ЗагрузитьССервераДополнений";
	КомандаОбновитьССервераДополнений = Команды.Добавить(ИмяКоманды);
	КомандаОбновитьССервераДополнений.Действие = ИмяКоманды;
	КомандаОбновитьССервераДополнений.Заголовок = "Обновить с сервера дополнений";
	КомандаОбновитьССервераДополнений.Картинка = БиблиотекаКартинок.МетаданныеВнешниеИсточникиДанных; // Необходимо установить картинку, доступную в конфигурации
	КомандаОбновитьССервераДополнений.Отображение = ОтображениеКнопки.КартинкаИТекст;
	
	КнопкаОбновитьССервераДополнений = Элементы.Вставить(
		ИмяКоманды,
		Тип("КнопкаФормы"),
		Элементы.КомандыВыгрузкиЗагрузки,
		Элементы.ЗагрузитьИзФайла);
	КнопкаОбновитьССервераДополнений.ИмяКоманды = ИмяКоманды;
	
	Элементы.ЗагрузитьИзФайла.Видимость = Пользователи.ЭтоПолноправныйПользователь();
	// +++ tura, 2023.01.18
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии Тогда
		// +++ tura, 2023.01.18
		ВариантыЗагрузки = Новый СписокЗначений;
		ВариантыЗагрузки.Добавить("ИзФайла", "Из файла");
		ВариантыЗагрузки.Добавить("ССервера", "С сервера");
		ОтветВидЗагрузки = Вопрос("Выберите вариант загрузки.", ВариантыЗагрузки,, "ССервера", "Выбор варианта загрузки");
		Если ОтветВидЗагрузки = "ССервера" Тогда
			ПодключитьОбработчикОжидания("ОбновитьССервераДополнений", 0.1, Истина);
		Иначе
...
// +++ tura, 2023.01.18
&НаКлиенте
Процедура ЗагрузитьССервераДополнений(Команда)
	ОбновитьССервераДополнений();	
КонецПроцедуры
...
// +++ tura, 2023.01.18
&НаКлиенте
Процедура ОбновитьССервераДополнений()
	...
	ПараметрыОтрытияФормы.Вставить("ТипДополнения", РаботаСДополнениямиИнтеграцияКлиентСервер.ТипДополненияДопОтчетОбработка());
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ОбновитьИзФайлаПослеВыбораФайла", ЭтотОбъект, ПараметрыРегистрации);
	ОткрытьФорму(
		"Обработка.РаботаСДополнениямиИнтеграция.Форма.ФормаПолученияДополнения",
