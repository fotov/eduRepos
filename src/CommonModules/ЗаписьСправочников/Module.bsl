// Модуль из СППР для формирования кодов иерархических справочников.
...
Процедура УстановитьПолныйКодСправочника(Объект, КоличествоРазрядов = 0) Экспорт
	...
	Если НЕ Объект.Родитель.Пустая() Тогда
		Объект.ПолныйКод = СокрЛП(Общегоназначения.ЗначениеРеквизитаОбъекта(Объект.Родитель, "ПолныйКод")) + "." + Код;
	Иначе
