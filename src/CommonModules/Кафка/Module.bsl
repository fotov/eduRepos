// За основу взято
// https://github.com/huxuxuya/KafkaConfluentRESTProxyAdapter1C
...
//  ФорматОбмена - Строка - Формат обмена, в котором должен происходить обмен с rest proxy, пока единственный поддерживемый это "json".
//  Пользователь - Строка - Пользователь при необходимости
//  Пароль - Строка - Пароль при необходимости
// 
...
Функция НовоеОписаниеСоединения(БазовыйАдрес, ФорматОбмена = "json", Пользователь = "", Пароль = "") Экспорт 
	...
	СвойстваСоединения.Вставить("Сессия", КоннекторHTTP.СоздатьСессию());
	Если ЗначениеЗаполнено(Пользователь) Тогда 
		СвойстваСоединения.Сессия.Аутентификация = Новый Структура("Пользователь, Пароль", Пользователь, Пароль);
	КонецЕсли;
...
Процедура ЗарегистрироватьПодписчика(Подписчик) Экспорт 
	...
	Ответ = КоннекторHTTP.Post(Адрес, ТелоЗапроса, Заголовки, ОписаниеСоединения.Сессия);
...
Функция ПолучитьСдвиги(Подписчик) Экспорт 
	...
	ОтветЗапроса = КоннекторHTTP.Get(Адрес, , Заголовки, ОписаниеСоединения.Сессия, ОписаниеСоединения.Сессия);
...
Функция ПолучитьСообщения(Подписчик) Экспорт 
	...
	Если ЗначениеЗаполнено(Подписчик.МаксимальныйРазмерОтветаБайт) Тогда
		ПараметрыЗапроса.Вставить("max_bytes", XMLСтрока(Подписчик.МаксимальныйРазмерОтветаБайт));
	КонецЕсли;

	ОтветЗапроса = КоннекторHTTP.Get(Адрес, ПараметрыЗапроса, Заголовки, ОписаниеСоединения.Сессия);
	
	МинимальныйСдвиг = Неопределено;
	...
	Если ОтветЗапроса.КодСостояния = 200 Тогда
		...
		Если ПолученныеДанные.Количество() > 0 Тогда
			...
			Для каждого ЗаписьДанных Из ПолученныеДанные Цикл
				...
				МинимальныйСдвиг = ?(МинимальныйСдвиг = Неопределено, Сдвиг, МинимальныйСдвиг);
				МаксимальныйСдвиг = ?(МаксимальныйСдвиг = Неопределено, Сдвиг, МаксимальныйСдвиг);
				
				МинимальныйСдвиг = Мин(МинимальныйСдвиг,Сдвиг);
	...
	ВыполнитьЛогированиеПолучитьСообщения(Подписчик, "Get", ОтветЗапроса, МинимальныйСдвиг, МаксимальныйСдвиг);
...
Процедура ПодтвердитьПолучение(Подписчик, СвойстваПодтверждения) Экспорт 
	...
	РезультатПодтверждениеОффсета = КоннекторHTTP.Post(Адрес, ПараметрОффсетов, Заголовки, ОписаниеСоединения.Сессия);
...
Процедура НазначитьПолучателюТопикИРаздел(Подписчик, Топик, Раздел) Экспорт 
	...
	РезультатПодтверждениеОффсета = КоннекторHTTP.Post(Адрес, ТелоЗапроса, Заголовки, ОписаниеСоединения.Сессия);
	
	ВыполнитьЛогированиеНазначитьПолучателюТопикИРаздел(Подписчик, "Post", ТелоЗапроса, РезультатПодтверждениеОффсета);

КонецПроцедуры

// Удалить подписчика. Выполняет удаление подписчика. Подписчика после выполнения операций необходимо удалять всегда.
// 
// DELETE /consumers/(string:group_name)/instances/(string:instance)
// 
// Параметры:
//  Подписчик - См. НовыйПодписчик
Процедура УдалитьПодписчика(Подписчик) Экспорт 
	...
	// Удаление подписчика
	РезультатУдаления =  КоннекторHTTP.Delete(Адрес, , Заголовки, ОписаниеСоединения.Сессия);  
	
	ВыполнитьЛогированиеУдалитьПодписчика(Подписчик, "Delete", РезультатУдаления);
...
Процедура Подписаться(Подписчик, Топик) Экспорт 
	...
	ТелоЗапросаПодписки = КоннекторHTTP.ОбъектВJson(ТопикиПодписки);
 	...
 	РезультатПодписки = КоннекторHTTP.Post(Адрес, ТелоЗапросаПодписки, Заголовки, ОписаниеСоединения.Сессия);
...
Процедура ОтправитьСообщения(Отправитель) Экспорт
	...
	Для Каждого ТопикСообщения Из СообщенияПоТопикам Цикл
		...
		РезультатОтправкиСообщения = КоннекторHTTP.Post(АдресТопика, СообщенияПодготовленные, Заголовки, ОписаниеСоединения.Сессия);
...
Процедура ВыполнитьЛогированиеУдалитьПодписчика(Подписчик, МетодHTTP,  Ответ)
...
Процедура ВыполнитьЛогированиеПолучитьСообщения(Подписчик, МетодHTTP, Ответ, МинимальныйСдвиг, МаксимальныйСдвиг)
	
	ОписаниеДействия = НСтр("ru = 'Получение сообщений'");
	ШаблонТелоЗапроса = НСтр("ru = '<empty> / Получены сообщения сдвигов: с %1 по %2'");
	ТелоЗапроса = СтрШаблон(ШаблонТелоЗапроса, МинимальныйСдвиг, МаксимальныйСдвиг);
...
Процедура ВыполнитьЛогированиеЗарегистрироватьПодписчика(Подписчик, МетодHTTP, ТекстЗапроса, Ответ)
	...
	ОписаниеДействия = НСтр("ru = 'Зарегистрирован подписчик'");
	ВыполнитьЛогированиеУниверсальное(Подписчик, МетодHTTP, ТекстЗапроса, Ответ, ОписаниеДействия);
...
Процедура ВыполнитьЛогированиеУниверсальное(Инициатор, МетодHTTP, ТекстЗапроса, Ответ, ОписаниеДействия, 
		ВыводитьТелоЗапроса = Истина, ВыводитьТелоОтвета = Истина)
...
Функция СвойстваОтправителя()
	...
	СтруктураОтправителя.Вставить("ОписаниеСоединения");
	СтруктураОтправителя.Вставить("СообщенияПоТопикам", СообщенияПоТопикам);

	Возврат СтруктураОтправителя;
