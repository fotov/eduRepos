...
Процедура ОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ФормаОтчета.ИмяФормы = "Отчет.АнализСинтаксическойПроверки.Форма" Тогда
		НаименованиеПравила = ОтчетыВызовСервераУК.ДанныеЭлементаРасшифровки(ФормаОтчета.ОтчетДанныеРасшифровки, Расшифровка, "Правило");
		Если НаименованиеПравила = Неопределено Тогда
			Возврат;
		КонецЕсли;
		Если СтрНайти(НаименованиеПравила, "http") Тогда
			// Это правило bsl-ls
			Возврат;
		КонецЕсли;
		// Расшифровка Правила АПК.
		СтандартнаяОбработка = Ложь;
		ОписаниеХТМЛ = ОтчетыВызовСервераУК.ПолучитьОписаниеПравилаПроверкиАПК(НаименованиеПравила);
		ОткрытьФорму("Отчет.АнализСинтаксическойПроверки.Форма.ФормаРасшифровкиПравила",
			Новый Структура("ОписаниеХТМЛ, Заголовок", ОписаниеХТМЛ, НаименованиеПравила),
			ФормаОтчета,
			НаименованиеПравила);
	КонецЕсли;
