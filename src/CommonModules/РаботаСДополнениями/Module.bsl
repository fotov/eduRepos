...
Процедура УстановитьДополнение(База, ВерсияДополнения) Экспорт
	...
	ИмяДополнения =СокрЛП(РеквизитыДополнения.ИмяОбъекта);
	
	Если СтрДлина(ИмяДополнения) > 80 Тогда
		ВызватьИсключение СтрШаблон("Длина наименования дополнения больше 80 символов (%1)", ИмяДополнения);
	КонецЕсли;
