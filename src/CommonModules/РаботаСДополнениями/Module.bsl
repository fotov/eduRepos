...
Процедура УстановитьДополнение(База, ВерсияДополнения) Экспорт
	...
	РеквизитыДополнения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВерсияДополнения, "Владелец.Наименование, Владелец.ТипДополнения");
	ИмяДополнения = РеквизитыДополнения.ВладелецНаименование; // Наименование Владельца (дополнения) берется из имени объекта
	
	Если СтрДлина(СокрЛП(ИмяДополнения)) > 80 Тогда
		ВызватьИсключение "Длина наименования дополнения больше 80 символов";
	КонецЕсли;
