...
Функция ДанныеПоЗадачеПроекта(ЗадачаМетеор, Проект, ТолькоАктивные = Ложь) Экспорт
	...
	|ИЗ
	|	РегистрСведений.ОчередьПолученияИзмененийОбъектовВерсийХранилища.СрезПоследних(
	|			,
	|			ВерсияХранилища В
	|				(ВЫБРАТЬ
	|					ВерсииПоЗадаче.ВерсияХранилища КАК ВерсияХранилища
	...
	|		И НЕ ЗадачиМетеорВерсийХранилища.ВерсияХранилища ЕСТЬ NULL
	...
	|				ПО СтатусыВерсийХранилищаСрезПоследних.Процесс = ПредварительнаяПроверка.Ссылка) КАК ПредварительнаяПроверкаОтветственный
	...
	ЗаполнитьПодсистемыИОтветственныхПоОбъектам(Проект, ДанныеПоЗадаче.ТаблицаОбъектов);

	Возврат ДанныеПоЗадаче;

КонецФункции

Функция ЗаполнитьПодсистемыИОтветственныхПоОбъектам(Конфигурация, ТаблицаОбъектов)
	...
	Запрос.Текст = 
		...
		|		ПО Объекты.Ссылка = СоставЛогическихПодсистем.ОбъектМетаданных";
	Запрос.УстановитьПараметр("ТаблицаОбъектов", ТаблицаОбъектов);
	Запрос.УстановитьПараметр("МассивИменОбъектов", ТаблицаОбъектов.ВыгрузитьКолонку("ИмяОбъекта"));
	ТаблицаОбъектов = (Запрос.Выполнить().Выгрузить());
	//РезультатЗапроса = Запрос.Выполнить();
	//ВыборкаОбъект = РезультатЗапроса.Выбрать();
	//Пока ВыборкаОбъект.Следующий() Цикл
	//	СтрокаОбъекта = ТаблицаОбъектов.Найти(ВыборкаОбъект.ПолноеИмя, "ИмяОбъекта");
	//	СтрокаОбъекта.Подсистемы = ВыборкаОбъект.ЛогическаяПодсистемаПредставление;
	//	СтрокаОбъекта.Ответственные = ВыборкаОбъект.Ответственный;
	//КонецЦикла;
