...
Функция ОтправитьСообщение(ПараметрыПисьма, ОписаниеОшибки = "", ТекстHTML = Неопределено, РазделительАдресов = ";",
	ОтправлятьТелоПисьмаПередТекстомHTML = Истина, Вложения = Неопределено) Экспорт
	
	Письмо = Новый ИнтернетПочтовоеСообщение;
	Письмо.Тема = ПараметрыПисьма.Тема;
	
	ИмяПолучателя = "";
	Если ПараметрыПисьма.АдресПолучателя = ПараметрыПисьма.ИмяПолучателя Тогда
		ИмяПолучателя = ""; // Если АдресПолучателя - список разделённый ";" - то отображаемое имя будет неверным
	Иначе
	...
	Если Вложения <> Неопределено Тогда 
		Для Каждого Вложение Из Вложения Цикл 
			РасширениеВложения = Неопределено;
			Попытка
				Если ТипЗнч(Вложения) = Тип("ТаблицаЗначений")
					И Вложения.Колонки.Найти("Расширение") <> Неопределено Тогда
					РасширениеВложения = Вложение.Расширение;
				КонецЕсли; 
			Исключение
			    РасширениеВложения = Неопределено;
			КонецПопытки; 
			ИмяФайла = ПолучитьИмяВременногоФайла(РасширениеВложения);
			Вложение.Значение.Записать(ИмяФайла);
			МассивКартинок.Добавить(ИмяФайла);
			Письмо.Вложения.Добавить(ИмяФайла, Вложение.Ключ);
			ТекстHTML = СтрЗаменить(ТекстHTML, Вложение.Ключ, ИмяФайла); 
		КонецЦикла;
	КонецЕсли;
	...
	Попытка
		Для Каждого ВременнаяКартинка Из МассивКартинок Цикл 
			УдалитьФайлы(ВременнаяКартинка);
		КонецЦикла;
	Исключение
		// сборщиком мусора удалятся
	КонецПопытки;
...
// Записывает новый документ ЭлектронноеПисьмо на основании данных ИнтернетПочтовоеСообщение
//
// Параметры:
//  ИнтернетПочтовоеСообщение	 - 	 - 
//
// +++ Турков, 2023.08.28
Процедура ЗаписатьЭлектронноеПисьмо(ИнтернетПочтовоеСообщение) Экспорт
		...
		Для Каждого Вложение Из ИнтернетПочтовоеСообщение.Вложения Цикл
			ПочтовоеВложение = ПочтовыеВложения.Добавить();
			ПочтовоеВложение.Расширение = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(Вложение.ИмяФайла);
			ПочтовоеВложение.Ключ = "att_" + XMLСтрока(ТекущаяУниверсальнаяДатаВМиллисекундах());
			ПочтовоеВложение.Значение = Вложение.Данные;
		КонецЦикла;

		Тексты = Новый Массив();
		Для Каждого Текст Из ИнтернетПочтовоеСообщение.Тексты Цикл
			Тексты.Добавить(Текст.Текст);
			ТипТекста = Текст.ТипТекста;
		КонецЦикла;
		Получатели = Новый Массив();
		Для Каждого Получатель Из ИнтернетПочтовоеСообщение.Получатели Цикл
			Получатели.Добавить(Получатель.Адрес);
		КонецЦикла;
		ДанныеПисьма = Новый Структура;
		ДанныеПисьма.Вставить("Получатели", СтрСоединить(Получатели, ";"));
		ДанныеПисьма.Вставить("Тема", ИнтернетПочтовоеСообщение.Тема);
		ДанныеПисьма.Вставить("Тело", СтрСоединить(Тексты, Символы.ПС));
