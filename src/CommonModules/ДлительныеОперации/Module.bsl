...
Функция ВыполнитьПоток(ПараметрыОперации, ПотокиПроцесса, Поток, Результат, ВыполнитьВФоне)
	...
	// +++ tura, 2023.03.29
	// Установка ключа фонового задания по ключу потока, если возможно
	// Поток - строка таблицы значений Потоки. Ключ потока передаётся при формировании параметров запуска в потоках.
	Попытка
		ПараметрыВыполнения.Вставить("КлючФоновогоЗадания", Строка(Поток.КлючПотока));
	Исключение
	КонецПопытки;
	// --- tura, 2023.03.29
